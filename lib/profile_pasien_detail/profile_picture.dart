import 'dart:io';
import 'package:aplikasi_reservasi_dokter_candra_safitri/profile_pasien_detail/change_profile_picture.dart';
  ...
  Future<void> _pickImage() async {
    ...
    if (pickedFile != null) {
      setState(() {
        ...
        Navigator.push(context, MaterialPageRoute(builder: (context) => ChangeProfilePicture(idPasien: widget.idPasien, selectedImage: _selectedImage!,)));
      });
    }
  }
...
Future<void> _updateImage() async {
  ...
  final apiUrl = 'https://reservasi.albertuscarlos-workspace.my.id/api/pasien/foto/update/';

  try {
    var request = http.MultipartRequest('POST', Uri.parse(apiUrl + widget.idPasien));
    request.fields['title'] = 'Static Title';

    var file = await http.MultipartFile.fromPath('foto_pasien', _selectedImage!.path);
    request.files.add(file);
          ...
          child: Column(
            children: [
              const SizedBox(
                ...
                SizedBox(
                  height: 400,
                  child: Image.network(fotoPasien)
                ),
